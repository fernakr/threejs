...
const MainScene = () => {
  ...
  const onAnimationFrameHandler = () => {  
    ...
    if (pug) {    
      ...
      if(pug.userData.move !== undefined && pug.position){
        ...
        //pug.position.x += pug.userData.move.forward * dt * pug.userData.move.speed;      
        //pug.rotation.y += -1 * pug.userData.move.turn * dt ;      
        if (pug.userData.move){
          if (pug.userData.move.forward) {
            ...
            pug.body.setVelocity(z, y, x)
          }
          pug.body.setAngularVelocityY(-1*pug.userData.move.turn)
        }else{
          pug.body.setAngularVelocityY(0);
          pug.body.setVelocity(0,0,0);
          console.log('stop');
        }
      

        pug.body.setAngularFactor(0, 0, 0)
        pug.body.setFriction(0.8)

        pug.body.needUpdate = true // this is how you update kinematic bodies
